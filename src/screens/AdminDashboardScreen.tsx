...
import { AdminGuestListScreen } from './AdminGuestListScreen';
import { useAuth } from '../state/AuthContext';

export function AdminDashboardScreen() {
  const { profile } = useAuth();
  const admin = profile?.role === 'admin';
  const sections = admin ? (['events', 'guests', 'people'] as const) : (['guests'] as const);
  const [section, setSection] = useState<'events' | 'guests' | 'people'>(admin ? 'events' : 'guests');
  return <SafeAreaView style={styles.root}><View style={styles.switcher}>{sections.map(item => <Pressable key={item} onPress={() => setSection(item)} style={[styles.button, section === item && styles.active]}><Text style={[styles.text, section === item && styles.activeText]}>{item.toUpperCase()}</Text></Pressable>)}</View><View style={styles.body}>{section === 'events' ? <AdminEventsScreen /> : section === 'people' ? <AdminUsersScreen /> : <AdminGuestListScreen />}</View></SafeAreaView>;
}
