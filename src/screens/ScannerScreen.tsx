...
export function ScannerScreen({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const { tickets, checkInTicket } = useAppState();
  const [locked, setLocked] = useState(false);
  const [result, setResult] = useState<CheckInResult | null>(null);
  const scan = (payload: string) => {
    if (locked) return;
    setLocked(true); setResult(checkInTicket(payload));
  };
  ...
  return <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}><SafeAreaView style={styles.root}>
    ...
    {!permission?.granted ? <View style={styles.permission}><Text style={styles.permissionTitle}>Camera access required</Text><Text style={styles.body}>Allow camera access to scan Vibe Districts QR tickets.</Text><PrimaryButton onPress={requestPermission}>ALLOW CAMERA</PrimaryButton></View> :
      <View style={styles.cameraWrap}><CameraView style={styles.camera} facing="back" barcodeScannerSettings={{ barcodeTypes: ['qr'] }} onBarcodeScanned={locked ? undefined : event => scan(event.data)} /><View pointerEvents="none" style={styles.frame} /></View>}
    <Text style={styles.hint}>Align the guest’s QR code inside the frame.</Text>
    {tickets.some(item => item.status === 'valid') && <Pressable onPress={() => scan(tickets.find(item => item.status === 'valid')!.qrPayload)} style={styles.demoButton}><Text style={styles.demoButtonText}>SIMULATE SCAN OF FIRST VALID TICKET</Text></Pressable>}
