Deno.serve(req => {
  ...
  const ticketSuccess = status === 'ticket-success';
  const ticketCancelled = status === 'ticket-cancelled';
  const title = success ? 'Payment received' : ticketSuccess ? 'Tickets purchased' : portal ? 'Membership updated' : 'Checkout cancelled';
  const message = success
    ? 'Return to the Vibe Districts app. Your membership will update after Stripe confirms the payment.'
    : ticketSuccess
      ? 'Return to the Vibe Districts app. Your QR tickets will appear after Stripe confirms payment.'
    : portal
      ? 'Return to the Vibe Districts app. Your billing changes will appear shortly.'
      : ticketCancelled ? 'No payment was taken. Reserved ticket inventory will be released.' : 'No payment was taken. You can return to the app and try again.';
